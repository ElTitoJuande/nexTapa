...
const itemSchema = new mongoose.Schema({
  ...
  // ── Modalidades: reemplaza type + isFree + price ─────────────────────────
  modalities: {
    type: [modalitySchema],
    validate: {
      validator: function (val) {
        if (!val || val.length === 0) return false;
        // Si isFree, el precio debe ser 0
        const hasInvalidFree = val.some(m => m.isFree && m.price > 0);
        if (hasInvalidFree) return false;
        // No puede haber labels duplicados (case insensitive)
        const labels = val.map(m => m.label.toLowerCase().trim());
        return labels.length === new Set(labels).size;
      },
      message: 'Las modalidades no son válidas (mínimo una, sin duplicados, gratis con precio 0)'
    },
    default: () => [{ label: 'Tapa', price: 0, isFree: false, available: true }]
  },
  // ─────────────────────────────────────────────────────────────────────────

  categories: [{
    type: String,
    // enum: ['carne', 'pescado', 'marisco', 'verduras', 'queso', 'embutido', 'frito', 'plancha', 'horno', 'guiso', 'ensalada', 'otro']
  }],
  allergens: [{
    type: String,
    // enum: ['gluten', 'lactosa', 'huevo', 'pescado', 'marisco', 'frutos_secos', 'soja', 'apio', 'mostaza', 'sesamo', 'sulfitos', 'altramuz', 'moluscos', 'cacahuetes']
  }],
  dietaryOptions: [{
    type: String,
    // enum: ['vegetariano', 'vegano', 'sin_gluten', 'sin_lactosa', 'sin_azucar']
  }],
...
// ── Virtuals de conveniencia ─────────────────────────────────────────────────

// ¿Tiene alguna modalidad gratuita activa?
itemSchema.virtual('isFree').get(function () {
  return this.modalities?.some(m => m.isFree && m.available) ?? false;
});

// Precio más bajo disponible (excluyendo gratis)
...
// Solo las modalidades activas (para el frontend público)
